...
impl ThreadController {
    ...
    pub async fn update_stats(&self) {
        ...
        stats.active_workflows = workflows.iter()
            .filter(|w| w.status == crate::orchestrator::WorkflowStatus::Running)
            .count();

        // Calculate overall system load
        stats.system_load = (stats.pool_utilization +
                           (stats.active_workflows as f32 / 10.0)) / 2.0;
    }
    ...
    pub async fn emergency_shutdown(&self) -> Result<(), String> {
        ...
        if let Some(pool) = &self.pool {
            let mut pool_lock = pool.lock().await;
            pool_lock.shutdown(std::time::Duration::from_secs(10)).await?;
        }
