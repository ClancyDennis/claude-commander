...
pub async fn execute_replan_step_v2(
    ...
    // Run until planning completes (with max iteration safety check)
    let mut loop_count = 0;
    const MAX_REPLAN_LOOPS: u32 = 100;

    loop {
        loop_count += 1;
        if loop_count > MAX_REPLAN_LOOPS {
            return Err(format!(
                "Replan step exceeded maximum iterations ({}). Agent may be stuck in a loop.",
                MAX_REPLAN_LOOPS
            ));
        }
