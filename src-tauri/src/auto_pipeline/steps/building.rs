...
pub async fn execute_building_step(
    ...
    // Run until execution completes (with max iteration safety check)
    let mut loop_count = 0;
    const MAX_BUILD_LOOPS: u32 = 100;

    loop {
        loop_count += 1;
        if loop_count > MAX_BUILD_LOOPS {
            return Err(format!(
                "Building step exceeded maximum iterations ({}). Agent may be stuck in a loop.",
                MAX_BUILD_LOOPS
            ));
        }
