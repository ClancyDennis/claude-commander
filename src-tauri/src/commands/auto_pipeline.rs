...
pub async fn start_auto_pipeline(
    ...
    tokio::spawn(async move {
        let _ = ctx
            .execute_pipeline(pipeline_id_clone, agent_manager, Arc::new(app_handle))
            .await;
    });
...
pub async fn get_auto_pipeline(
    ...
    eprintln!(
        "[get_auto_pipeline] Pipeline status={:?}, steps={}",
        pipeline.status,
        pipeline.steps.len()
    );
    ...
    // Fetch ALL outputs for this pipeline from the database
    {
        ...
        match runs_db.query_agent_outputs(filters).await {
            Ok(db_outputs) => {
                eprintln!(
                    "[get_auto_pipeline] Found {} total outputs in database for pipeline",
                    db_outputs.len()
                );

                // Convert AgentOutputRecord to AgentOutputEvent
                let all_outputs: Vec<crate::types::AgentOutputEvent> = db_outputs
                    .into_iter()
                    .map(|record| {
                        let metadata_value: Option<serde_json::Value> = record
                            .metadata
                            .as_ref()
                ...
                for step in &mut pipeline.steps {
                    if let Some(ref agent_id) = step.agent_id {
                        let step_outputs: Vec<_> = all_outputs
                            .iter()
                        ...
                        if !step_outputs.is_empty() {
                            eprintln!(
                                "[get_auto_pipeline] Assigning {} outputs to step {:?}",
                                step_outputs.len(),
                                step.role
                            );
                ...
                eprintln!(
                    "[get_auto_pipeline] Returning pipeline with {} total outputs",
                    all_outputs.len()
                );
            }
            Err(e) => {
