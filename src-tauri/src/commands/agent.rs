...
pub async fn create_agent(
        ...
        if !instruction_files.is_empty() {
            ...
            for instruction_file in instruction_files {
                ...
                if let Some(existing_skill) = skill_generator::find_existing_skill_for_instruction(
                    ...
                    // Emit skipped event
                    let _ = app_handle.emit("skill_generation:skill_skipped", serde_json::json!({
                    ...
                    match skill_generator::generate_skill_from_instruction(
                        ...
                        Err(e) => {
                            ...
                            if skill_generator::is_auth_error(&e) {
                                eprintln!("⚠ API key issue detected, creating basic skill from: {}", instruction_file);
                                ...
                                match skill_generator::create_fallback_skill_from_instruction(
                                    ...
                                    Err(fallback_err) => {
                                        eprintln!("✗ Failed to create basic skill: {}", fallback_err);
                                        failed += 1;
            ...
            eprintln!("Skill generation completed: {} generated, {} skipped, {} failed", completed, skipped, failed);
        }
    }
    ...
    let agent_id = {
        ...
        manager.create_agent_with_skills(
            working_dir.clone(),
            github_url,
