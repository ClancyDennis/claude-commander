...
export interface OrchestratorToolCall {
  ...
  iteration: number;
  timestamp: number;
  completed?: boolean;  // Track if tool finished
  completion_time?: number; // When it completed
}
...
export interface PipelineHistoryBundle {
  tool_calls: OrchestratorToolCallRecord[];
  state_changes: OrchestratorStateChangeRecord[];
