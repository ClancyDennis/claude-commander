<script lang="ts">
  ...
  // Unified output type for display
  type UnifiedOutput = {
    stage: string;
    output_type: string;
    content: string;
  ...
  // Combine all outputs from all steps AND orchestrator events
  let allOutputs = $derived.by(() => {
    if (!pipeline) return [];
    const outputs: UnifiedOutput[] = [];

    // Add agent outputs from steps
    console.log('[AutoPipelineView] Processing pipeline steps:', pipeline.steps.length);
    for (const step of pipeline.steps) {
      console.log(`[AutoPipelineView] Step ${step.role}: has output=${!!step.output}, has agent_outputs=${!!step.output?.agent_outputs}, count=${step.output?.agent_outputs?.length || 0}`);
  ...
  // Map orchestrator state to which stage should be animated
  let orchestratorActiveStage = $derived.by(() => {
    const state = $orchestratorCurrentState;

    // Map orchestrator states to stage indices (0=Planning, 1=Building, 2=Verifying)
    switch (state) {
      case 'Planning':
        return 0; // Planning stage
      case 'ReadyForExecution':
        return 0; // Still planning, but approved
      case 'Executing':
        return 1; // Building stage
      case 'Verifying':
        return 2; // Verifying stage
      case 'Completed':
      case 'GaveUp':
        return null; // No active stage
      default:
        return null; // Unknown state or Idle
    }
  });

  // Track tool execution state based on completion flag
  ...
  // Get current tool status with error state
  let currentToolStatus = $derived.by(() => {
    if (!currentTool) return 'idle';
    if (!currentTool.completed) return 'executing';
    if (currentTool.is_error) return 'error';
    return 'completed';
  });
  ...
  async function loadPipeline(id: string) {
    ...
    if (storedPipeline) {
      pipeline = storedPipeline;
      loading = false;
      // Still fetch history even if pipeline is in store
  ...
  async function loadPipelineHistory(id: string) {
    try {
      ...
      console.log('[AutoPipelineView] Loaded pipeline history:', pipelineHistory);
      console.log('[AutoPipelineView]   - tool_calls:', pipelineHistory.tool_calls?.length || 0);
      console.log('[AutoPipelineView]   - state_changes:', pipelineHistory.state_changes?.length || 0);
      console.log('[AutoPipelineView]   - decisions:', pipelineHistory.decisions?.length || 0);
    } catch (e) {
      console.error('[AutoPipelineView] Failed to load pipeline history:', e);
      // Don't set error - just log it, as this is a non-critical enhancement
    }
  }
